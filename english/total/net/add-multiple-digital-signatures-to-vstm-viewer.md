...
############################# Head ############################
head_title: "Add Multiple Digital Signatures to VSTM Viewer in .NET"
head_description: "Sign macro-enabled drawing template created by Visio (VSTM) file with multiple digital signature types (Barcode, Image, Metadata, QR-Code, Stamp, Text, Form Field) in C# .NET applications."

############################# Header ############################
title: "Add Multiple Signatures to VSTM Viewer"
description: "Sign macro-enabled drawing template files created by Visio (VSTM) with different digital signature types (Barcode, Image, Metadata, QR-Code, Stamp, Text, Form Field). View the signed file in HTML, Image or PDF viewer in .NET (C#, ASP.NET, VB.NET, .NET Core) applications."
...
steps:
    enable: true
    title_left: "Put multiple signatures to VSTM in .NET"
    content_left: |
        [GroupDocs.Total](https://products.groupdocs.com/total/net/) makes it easy for developers to insert multiple digital signatures to macro-enabled drawing template documents created by Visio (VSTM) by adding a few lines of C# .NET code.
    ...
    code: |
        ```cs
        // Digitally sign VSTM file with multiple signatures in .NET
        ...
        using (Signature signature = new Signature("input.vstm"))
          {
            ...
            signature.Sign("signed.vstm", options);
